...
The following example shows a valid live master playlist as input to AWS Elemental MediaTailor\.
...
The following example shows a media playlist as input to AWS Elemental MediaTailor\. Note the `EXT-X-CUE-OUT` and `EXT-X-CUE-IN` tags describing ad break opportunities\.
